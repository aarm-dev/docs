...
- **DEFER → escalation** — Action was [deferred](/components/deferral-service) due to insufficient context, and automated resolution was unable to collect enough information; the deferral is escalated to a human approver
...
<Warning>
  If identity claims (tokens, sessions, role assignments) expire before approval is granted, the system must re-validate them before executing. Stale identity must never be used to authorize execution. See [R5: Identity Binding](/conformance/requirements#r5-identity-binding).
</Warning>
